...
def cornersHeuristic(state, problem):
    ...
    a = [x for x in corners if food[x[0]][x[1]]]
    if len(a)== 0:
        return 0
    c = 0
    b = util.manhattanDistance(state[0],a[0])
    for x in range(len(a)-1):
        c += util.manhattanDistance(state[0],a[x+1])
    ...
    #for i in corners:
    #    if(food[i[0]][i[1]]):
    #        result.append(util.manhattanDistance(state[0],i))
#
#    #result.sort()
#    #if len(result) == 0:
#    #    return 0
#
    #return result.pop(0)
        ...
        #count += util.manhattanDistance(corners[i],corners[i+1])
...
def foodHeuristic(state, problem):
    ...
    for food_position in food_list:
        maxvalue = max(maxvalue, mazeDistance(position, food_position, startState))
    return maxvalue
